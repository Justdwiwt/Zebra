...
public class APP02Servlet extends HttpServlet {
    ...
    @SuppressWarnings({"UnusedAssignment", "Convert2Diamond", "ConstantConditions"})
    public void doGet(HttpServletRequest request, HttpServletResponse response)
            throws IOException {
        String app01name = new String(request.getParameter("app01name").getBytes("iso8859-1"), StandardCharsets.UTF_8);
        ...
        try {
            ...
            //处理结果 -- 图例
            StringBuilder legendBuf = new StringBuilder();
            //处理结果 -- 数据
            StringBuilder seriesBuf = new StringBuilder();

            int sum = 0;
            for (Map<String, Object> m : list) {
                String app02Name = APP02.getName(((Long) m.get("appsubtype")).intValue());
                legendBuf.append("'").append(app02Name).append("',");

                int data = ((BigDecimal) m.get("su")).intValue();
                sum += data;
                seriesBuf.append("{value:").append(data).append(", name:'").append(app02Name).append("'},");
            }
